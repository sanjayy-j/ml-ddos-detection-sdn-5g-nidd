...
> **Status: complete.** Stages A-L are finished and committed. Random
> Forest, SVM and the 1-D CNN are trained and evaluated under a common
> validation-selected FPR <= 1% operating point (§16D), with a secondary
> unseen-capture experiment in §16H. Results below are real experimental
> results generated by the scripts in `experiments/` and stored under
> `results/`; see §0 for the artifact index and §16 to reproduce them.
>
> Scope note: the primary evaluation is **within-capture (within-session)**
> — see §8.3 for the exact wording that should be used to describe it.

---

## 0. Results and artifact index

All numbers in this document are generated by the scripts in
`experiments/` and stored under `results/`. These files are the
authoritative source; the tables here are summaries of them.

| Artifact | Contents |
|---|---|
| [`results/evaluation/three_model_fpr1pct_summary.csv`](results/evaluation/three_model_fpr1pct_summary.csv) | Canonical three-model comparison at the frozen operating point |
| [`results/evaluation/three_model_confusion_matrices.csv`](results/evaluation/three_model_confusion_matrices.csv) | Test confusion counts per model |
| [`results/evaluation/three_model_per_attack_type.csv`](results/evaluation/three_model_per_attack_type.csv) | Per-attack-type recall, all models |
| [`results/evaluation/three_model_fpr_budget.csv`](results/evaluation/three_model_fpr_budget.csv) | Validation recall at 0.1/1/5/10% FPR budgets |
| [`results/evaluation/metric_consistency_audit.csv`](results/evaluation/metric_consistency_audit.csv) | Metrics re-derived from confusion counts |
| [`results/evaluation/model_comparison.md`](results/evaluation/model_comparison.md) | Stage K report-ready comparison |
| [`results/unseen_capture/unseen_capture_comparison.md`](results/unseen_capture/unseen_capture_comparison.md) | Stage L report-ready secondary experiment |
| [`results/unseen_capture/unseen_capture_preparation.json`](results/unseen_capture/unseen_capture_preparation.json) | Stage L split, provenance, novelty statistics |
| [`results/split_manifest.csv`](results/split_manifest.csv) | 20 capture blocks: base station, attack types, per-split counts |
| [`results/feature_decisions.csv`](results/feature_decisions.csv) / [`.md`](results/feature_decisions.md) | Per-column KEEP/DROP decision with evidence |
| [`results/feature_order.json`](results/feature_order.json) | The locked 67-feature order |
| [`results/preprocessing_metadata.json`](results/preprocessing_metadata.json) | Split sizes, duplicate/conflict analysis, empirical ceiling |
| [`results/leakage_audit.json`](results/leakage_audit.json) | Stage D leakage checks |
| `results/{random_forest,svm,cnn}/` | Per-model run records, searches, curves, per-attack tables |
| `results/unseen_capture/{rf,svm,cnn}/` | Stage L per-model records, per-block metrics, curves |
| [`results/plots/`](results/plots/) | Nine figures + [`plot_manifest.json`](results/plots/plot_manifest.json) provenance |

Large generated matrices and trained models live in
`M1-supervised-ml/data/processed/` (**git-ignored**, ~691 MB across the
primary and Stage L representations) and are regenerated by the CLIs
in §16.
...
### 8.4 Secondary unseen-capture experiment — implemented in Stage L

Because the primary protocol cannot measure unseen-capture
generalisation, a **secondary** experiment was defined here and is
**implemented in Stage L (§16H)**. It does not affect or replace the
primary protocol.

A six-block candidate (10, 11, 12, 13, 15, 17) was proposed at this
stage and was subsequently **rejected** by the Stage L design audit:
6.787 pp class-prior shift, all blocks from a single base station, no
HTTPFlood, and 69.66% feature-vector repetition. The audit searched all
42,093 feasible whole-block holdouts on dataset structure alone and
selected instead:

| Property | Accepted holdout (Stage L) |
|---|---|
| Held-out blocks | **1, 3, 10, 12, 15, 16, 17, 19** |
| Development blocks | **0, 2, 4, 5, 6, 7, 8, 9, 11, 13, 14, 18** |
| Held-out rows | 173,527 (14.3% of dataset) |
| Class-prior shift | ~0.001 pp |
| Attack types held out | 7 (incl. HTTPFlood); both base stations |
| Rows whose vector occurs in development | 45.17% |

A leave-one-attack-out variant is also definable for unseen-*attack*
generalisation, but it changes the research question from "detect known
attack classes" to "detect novel attacks"; it was **not** performed.
See §16H for the protocol, results and interpretation constraints.
...
As of this writing the `M2-entropy/` and `M3-sampled-incremental/`
directories contain only their original scaffolding and a `.gitkeep`
file, so no cross-methodology comparison has been carried out. Those
methodologies are owned by other team members and are outside the scope
of M1.
...
M1-supervised-ml/
├── config/
│   ├── m1_config.yaml                  # primary experiment configuration
│   ├── m1_config.synthetic.yaml        # loader/validation test config
│   └── m1_config_unseen_capture.yaml   # Stage L secondary experiment
├── preprocessing/
...
│   ├── splitting.py                    # block detection + primary split + manifest
│   ├── transformers.py                 # skew-gated log1p + ColumnTransformer
│   ├── reporting.py                    # feature decision report
│   ├── block_holdout.py                # Stage L whole-block partitioning
│   └── pipeline.py                     # orchestration, artifacts, leakage audit
├── models/
│   ├── random_forest.py                # RF factory (config-driven)
│   ├── svm.py                          # Nystroem RBF map + LinearSVC
│   └── cnn.py                          # 1-D CNN (TensorFlow/Keras)
├── training/
│   └── metadata.py                     # per-split analysis metadata (Attack Type/Tool)
├── evaluation/
│   ├── metrics.py                      # binary metrics, explicit FPR, per-group
│   ├── thresholds.py                   # locked FPR <= 1% operating-point protocol
│   ├── consolidate.py                  # Stage I consolidation + metric audit
│   ├── plots.py                        # Stage J figure generation
│   ├── comparison.py                   # Stage K facts + claim checking
│   └── novelty.py                      # Stage L feature-vector novelty
├── experiments/                        # 12 CLI entrypoints — see §16
├── tests/                              # 249 tests; no dependency on the 263 MB CSV
└── results/                            # generated artifacts (tracked, small) — see §0
```

Note: `models/random-forest/`, `models/svm/` and `models/cnn-1d/` remain
as empty Stage-A scaffolding directories. Hyphens are not valid Python
identifiers, so the importable modules are the underscore files listed
above.

Processed matrices and trained models go to
`M1-supervised-ml/data/processed/` (**git-ignored**, ~691 MB across the
primary and Stage L representations) and are regenerated by the CLIs in
§16.
...
No repository-wide dependency file exists; the M1 dependency set is
recorded here (a shared file is deferred until M2/M3 are developed):

```
pandas  numpy  scikit-learn  pyyaml  joblib  pytest
tensorflow   # 2.21.0 — Stage H / Stage L 1-D CNN
matplotlib   # 3.11.1 — Stage J figures
```

Verified environment: Python 3.13.7, numpy 2.5.2, pandas 3.0.5,
scikit-learn 1.9.0, scipy 1.18.1, joblib 1.5.3, TensorFlow 2.21.0,
Keras 3.15.1, matplotlib 3.11.1, on Windows (AMD64).

> **Windows / TensorFlow caveat.** TensorFlow provides no GPU support on
> native Windows from 2.11 onward, so the CNN trains and scores on
> **CPU** here regardless of installed CUDA. TF also emits
> `WARNING:tensorflow: ... GPU support is not available on native
> Windows`; this is expected. Installing TensorFlow upgrades `protobuf`
> to 7.36.1, which pip reports as incompatible with `grpcio-status` and
> `proto-plus` if they are present; neither is used anywhere in M1.

### Dataset

`data/raw/Combined.csv` must be obtained separately and kept **local**.
It is git-ignored and must never be committed. Set its location via
`dataset.raw_path` in the config (repo-relative) or `--dataset-path`.

### Reproduction order

All commands are run **from the repository root**. Every script accepts
`--config`; the additional flags shown are the actual ones each script
defines.

**1 — Prepare the primary representation (Stage D/E)**
...
Flags: `--dataset-path PATH` (override the configured path), `--no-save`
(run validation and the leakage audit without writing artifacts),
`--nrows N` (smoke test only — **not** a valid experimental run).

**2 — Train and evaluate the three models (Stages F/G/H)**

Each trains on the training split, selects hyperparameters and the
threshold on validation, and then evaluates the test split exactly once.
...
Flags: `train_random_forest.py` accepts `--skip-search` (use the
configured hyperparameters unchanged) and `--rebuild-metadata`;
`train_svm.py` accepts `--rebuild-metadata`; `train_cnn.py` accepts
`--config` only.

Approximate cost on the reference machine, taken from the saved run
records (`timings_seconds`): RF 31 s train / 966 s grid (~16 min); CNN
160 s train / 25,500 s grid; SVM 3,094 s train / 14,358 s grid (~4 h).
The CNN grid total reflects environmental machine stalls rather than
computation — two fits of one architecture logged 16,748 s and 3,577 s
where the same architecture at a different learning rate took 656 s
(§16C) — so ~2.4 h excluding the largest outlier is more representative.
Machine load varies all of these substantially.

**3 — Authorised secondary RF evaluation at the common operating point**

The Stage F RF result used an earlier F1-oriented threshold rule; this
re-evaluates the **same saved forest** at the locked FPR <= 1% point. It
does not retrain and writes only additive artifacts.

```bash
python M1-supervised-ml/experiments/evaluate_rf_at_fpr.py
```

**4 — Consolidate the evaluation (Stage I)**

```bash
python M1-supervised-ml/experiments/consolidate_evaluation.py
```

**5 — Export curves and generate figures (Stage J)**

`export_curves.py` re-scores the test split with the **saved** RF and SVM
models purely to persist ROC/PR curve points; it selects no threshold and
refuses to overwrite existing curve files.
...
**6 — Final comparison (Stage K)**

```bash
python M1-supervised-ml/experiments/generate_comparison.py
```

**7 — Secondary unseen-capture experiment (Stage L)**

Uses `config/m1_config_unseen_capture.yaml`. Preparation fits a **fresh**
preprocessor on development-train only; each model is trained from
scratch and the held-out blocks are scored exactly once.
...
`--model` accepts `cnn`, `rf` or `svm` and is required.

### Tests

The suite uses synthetic fixtures only and does not need the 263 MB CSV:

```bash
cd M1-supervised-ml && python -m pytest tests/ -q
```

Current baseline: **249 tests**.
...
## 16I. Consolidated limitations

A single list of everything that constrains how M1's results may be read.
Each item is documented in detail in the section referenced.

**Data and representation**

1. **`Combined.csv` contains no source IP, destination IP, port or
   wall-clock timestamp field** (verified across all 52 columns; `RunTime`
   is a flow *duration*, not a timestamp). M1 therefore performs no
   endpoint-identity, port-based or true-temporal analysis, and capture
   ordering is available only as row position within a block via the
   Argus `Offset` column.
2. **Per-flow representation.** Each prediction is one Argus flow record;
   M1 has no window- or session-level aggregation (§11).
3. **Substantial exact-feature repetition** — only 345,621 distinct
   vectors for 1,215,890 rows; 72.97% of test rows share a feature vector
   with training data (§8.2).
4. **Label conflicts and an empirical ceiling** — 33,709 feature vectors
   carry both labels, giving 281,533 forced errors, an empirical ceiling
   of 76.8455% overall and 78.6258% on the test split. This is an
   *empirical* ceiling for this exact-vector representation, **not** a
   theoretical limit for every model or richer feature set (§12).
5. **UDPFlood ambiguity dominates** — one vector occurs 271,476 times as
   159,875 UDPFlood and 111,601 Benign, and UDP flood recall is
   0.074-0.097 across all three families (§16D).

**Evaluation scope**

6. **The primary protocol is within-capture (within-session).** All 20
   capture blocks contribute to train, validation and test. It does
   **not** measure unseen-capture, unseen-session, unseen-base-station or
   unseen-attack generalisation (§8.3).
7. **Stage L is a secondary experiment with a different attack
   composition.** UDPFlood is absent from its holdout and the holdout is
   nearly conflict-free (0.0046% of rows, versus 56.2% for the primary
   test), so Stage L results must **not** be read as a straight
   improvement over the primary test, nor as evidence of universal
   unseen-capture generalisation (§16H).
8. **No unseen-attack-type experiment was performed.** Stage L holds out
   unseen *captures* of attack types that remain represented in training.
9. **Accuracy alone is not a valid ranking metric** — §16A.1 shows
   near-ceiling accuracy coexisting with a 54.4% false-positive rate.

**Measurement and deployment**

10. **Timing figures are amortised batch throughput**, not single-flow
    latency, from single unreplicated runs on a load-variable machine
    (repeated timing of the identical RF scoring call produced 0.0011 and
    0.0017 ms/flow). Treat them as indicative of relative cost only
    (§16D).
11. **No real SDN deployment validation.** This is an offline flow-level
    study on a public dataset. It does not cover controller integration,
    live telemetry collection cost, flow-table behaviour or mitigation
    actions, and it does **not** establish real-world 5G/SDN deployment
    performance.
12. **Results are specific to the evaluated 67-feature representation**
    and to the 5G-NIDD `Combined.csv` capture.
